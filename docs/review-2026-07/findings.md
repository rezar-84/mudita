...
- **What:** `DecorationOverlay` renders `decoration.svgMarkup` with
  `dangerouslySetInnerHTML`. `sanitiseSvg()` is only applied at *upload* time.
...
- **Fix direction:** Sanitize on the way *in*, not only on upload. Re-run
...
- **Fix direction:** `git rm --cached .env`, add `.env` (and `.env.*` except
  `.env.example`) to `.gitignore`, commit a `.env.example` with key *names* only.
...
- **Fix direction:** Decide the v1 model explicitly:
  ...
  Until (b), the "order" should be framed as a *quote/reservation*, not a purchase.
...
- **What:** `price_try`, `subtotal_try`, `total_try` are stored exactly as the
  ...
  A user can place an order for ₺1. Blast radius is limited *only* because there is
...
- **What:** The route guards are `ssr:false` client redirects. The *real*
...
- **What:** `useBlocker` fires a `window.confirm` on *any* nav away (except
