...
export function DecorationProperties() {
  ...
  if (!d) {
    return <p className="text-sm text-muted-foreground">{t("decoNotFound")}</p>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm" onClick={() => updateDecoration(d.id, { locked: !d.locked })}>
          {d.locked ? <Unlock className="mr-1.5 h-3.5 w-3.5" /> : <Lock className="mr-1.5 h-3.5 w-3.5" />}
          {d.locked ? t("unlock") : t("lock")}
        </Button>
        <Button variant="outline" size="sm" onClick={() => updateDecoration(d.id, { hidden: !d.hidden })}>
          {d.hidden ? <Eye className="mr-1.5 h-3.5 w-3.5" /> : <EyeOff className="mr-1.5 h-3.5 w-3.5" />}
          {d.hidden ? t("show") : t("hide")}
        </Button>
        <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => removeDecoration(d.id)}>
      ...
      <div>
        <Label className="mb-2 flex items-center justify-between text-sm">
          {t("layerSize")} <span className="text-muted-foreground">{d.sizePct}%</span>
        </Label>
      ...
      <div>
        ...
        <Slider min={-180} max={180} step={5} value={[d.rotation]} onValueChange={([v]) => updateDecoration(d.id, { rotation: v })} />
        <div className="mt-2 flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => updateDecoration(d.id, { rotation: ((d.rotation - 90 + 540) % 360) - 180 })}>
            <RotateCcw className="mr-1 h-3.5 w-3.5" /> -90°
          </Button>
          <Button variant="outline" size="sm" onClick={() => updateDecoration(d.id, { rotation: ((d.rotation + 90 + 540) % 360) - 180 })}>
            <RotateCw className="mr-1 h-3.5 w-3.5" /> +90°
          </Button>
          <Button variant="outline" size="sm" onClick={() => updateDecoration(d.id, { rotation: 0 })}>
            0°
          </Button>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant={d.flipX ? "default" : "outline"} size="sm" onClick={() => updateDecoration(d.id, { flipX: !d.flipX })}>
            <FlipHorizontal2 className="mr-1 h-3.5 w-3.5" /> {t("horizontal")}
          </Button>
          <Button variant={d.flipY ? "default" : "outline"} size="sm" onClick={() => updateDecoration(d.id, { flipY: !d.flipY })}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Slider min={-45} max={45} step={1} value={[d.x]} onValueChange={([v]) => updateDecoration(d.id, { x: v })} />
        </div>
        <div>
          <Label className="mb-2 flex items-center justify-between text-xs">
            Y <span className="text-muted-foreground">{d.y}</span>
          </Label>
          <Slider min={-45} max={45} step={1} value={[d.y]} onValueChange={([v]) => updateDecoration(d.id, { y: v })} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between rounded-lg border border-border p-3">
        <div>
          <p className="text-sm">{t("decoShowLabel")}</p>
          <p className="text-[11px] text-muted-foreground">{t("layerExcludeDesc")}</p>
        </div>
        <Switch checked={!d.hidden} onCheckedChange={(v) => updateDecoration(d.id, { hidden: !v })} />
      </div>
    </div>
  );
}
