/**
 * Curated SVG path library for neon decorations.
 * Each entry has a single `path` rendered in a 24×24 viewBox so they line up
 * visually. Paths are intentionally simple silhouettes that look right when
 * given a multi-layer neon glow filter (same as the text).
 */
export interface DecorationPreset {
  id: string;
  label: string;
  category: "love" | "nature" | "food" | "music" | "weather" | "symbol" | "fun";
  /** Single SVG path, drawn in a 24×24 viewBox. */
  path: string;
  viewBox?: string;
}
...
export const DECORATIONS: DecorationPreset[] = [
  ...
  },
  {
    ...
    label: "Parıltı",
    category: "symbol",
    path: "M12 2 13 10 21 11 13 12 12 21 11 12 3 11 11 10z",
  },
  {
    id: "crown",
    label: "Taç",
